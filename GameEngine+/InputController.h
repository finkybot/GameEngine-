...
public:
	...
	void Init(InputAction quitAction, sf::RenderWindow* window);	// Initializes the input controller with the specified quit action and window reference, allowing it to handle quit events and poll for input events from the given window
	void Update(uint32_t deltaT);									// Updates the input controller by polling for SFML events and triggering the appropriate actions based on user input, such as key presses, mouse movements, and window events. It takes the delta time since the last update as a parameter for potential use in input handling logic (e.g., for timing-based input actions).
