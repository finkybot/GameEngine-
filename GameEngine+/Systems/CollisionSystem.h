...
private:
	...
	bool IsColliding(const Entity* entity1, const Entity* entity2)
		const; // Checks if two entities are colliding based on their positions and radii. It calculates the distance between the centers of the two entities and compares it to the sum of their radii to determine if a collision is occurring.
	...
	void BounceEntities(Entity* entity1, Entity* entity2)
		const; // Applies an elastic collision response to bounce two allied entities apart. It calculates the normal vector between the two entities, computes the relative velocity, and updates the velocities of both entities based on the collision response formula for elastic collisions.
	bool AreEnemies(const Entity* entity1, const Entity* entity2)
		const; // Checks if two entities are enemies based on their types/tags. It compares the EntityType of both entities and returns true if they are different (indicating they are enemies) or false if they are the same (indicating they are allies).
};
