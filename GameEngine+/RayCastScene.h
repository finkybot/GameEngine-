...
/////////////////////////////////
//	|	RayCastScene class - A simple scene to test the tilemap component and TileSystem. This scene allows for interactive raycasting and tile editing, providing visual debug overlays to help visualize the raycasting process and tile states. It serves as a testing ground for the tilemap functionality and can be used to 
//	|	experiment with different tile configurations and raycasting scenarios. (Note: This scene is not intended to be a full level editor, but rather a simple testbed for tilemap features.)
...
public:
	// The Update method will handle logic for processing user input, performing raycasts based on mouse interactions, and updating any necessary state for rendering. The Render method will handle drawing the tilemap and any visual debug overlays to the screen. The HandleEvent method will process SFML events for user input, 
	// while the OnEnter and OnExit methods can be used to set up and clean up the scene as needed.
	void Update(float deltaTime) override; 
	void Render() override;
	void DoAction() override;

	// Event lifecycle methods for the RayCastScene class. These methods will handle processing input events, setting up the scene when it becomes active, and cleaning up when it exits. The LoadResources and UnloadResources methods can be used to manage  any resources specific to this scene, while the 
	// InitializeGame method can be used to set up the initial game state (e.g., spawning a test tilemap).
...
private:
	...
	// Input processing helpers for handling specific key presses (e.g., Left Ctrl for toggling preview mode, a debug toggle key, Escape for closing the window, and a save key for saving the tilemap). These methods will be called from the HandleEvent method to process user input and update the scene state accordingly.
...
private:
	// Reference to the SFML render window for rendering and context and a 2D grid of tile values for rendering. The m_debugLines vector will store pairs of Vec2 representing the start and end points of lines to draw for raycasts, while the m_debugPoints vector will store hit points from raycasts.
