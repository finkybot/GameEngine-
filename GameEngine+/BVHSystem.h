...
/////////////////////////////////
//	|	BVHSystem class - manages the Bounding Volume Hierarchy (BVH) tree for efficient spatial queries, such as raycasting. It provides methods for rebuilding the BVH tree based on a list of entities, performing raycasts against the tree, and retrieving the root node of the tree. The BVH system optimizes collision detection and 
//	|	intersection tests with entities in the scene by organizing them in a hierarchical structure based on their bounding boxes.
...
public:
	...
	// Rebuild - rebuilds the BVH tree based on the provided list of entities. This method takes a vector of pointers to Entity objects and constructs a new BVH tree that organizes the entities in a way that optimizes spatial queries, such as raycasting. The rebuilt tree will allow for efficient 
	// collision detection and intersection tests with the entities in the scene.
...
private:
	...
	// RaycastNode - performs a raycast against the BVH tree starting from the given node. This method recursively traverses the tree, testing the ray against the bounding boxes of each node and checking for intersections with the entities contained in leaf nodes.
