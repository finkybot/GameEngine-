...
private:
	...
	/////////////////////////////////
	//	|	ExplosionVA struct definition. This struct represents a single explosion visual effect in the music visualizer scene. It contains properties for the explosion's center position, radius, age, lifetime, color, vertical velocity, and alive state.
	//	|	The ExplosionVA struct is used to manage and update the state of individual explosions, allowing for dynamic visual effects that respond to the music being played. Each explosion can be spawned, updated, and rendered independently, with its properties determining how it behaves over time.
	//	|_______________________________________________________________________
	struct ExplosionVA {
		sf::Vector2f center;
		float radius;
	...
	// Equalizer bar management: pre-allocated bars that are lit based on spectrum, initialize visualizer bars: visualCount = number of bars drawn; independent from spectrum band count
	...
	// 3D Audio positioning controls for music entity (experimenting with spatial audio) These will be initialized based on the actual window size in LoadMusicFromPath
	...
	// Temporary drawable storage for render queue. These are cleared at the start of Render()  and repopulated with frame-specific drawables for enqueueing to the engine render queue.
