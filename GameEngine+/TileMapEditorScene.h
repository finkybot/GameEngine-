...
class TileMapEditorScene : public Scene
{
	// Public Methods   
public:
	TileMapEditorScene(GameEngine& engine, sf::RenderWindow& win, EntityManager& entityManager); // Constructor - initializes the tile map editor scene with references to the game engine, render window, and entity manager
	~TileMapEditorScene() override; // defaulted since we don't have any special cleanup logic, but we could add it if needed in the future
...
private:
	...
	void ProcessInput(); // Handle user input for editing the tile map, such as mouse clicks for toggling tiles, keyboard shortcuts for saving/loading, etc.
	void ToggleTileAt(int tx, int ty, bool setSolid); // Toggle the tile at the given tile coordinates (tx, ty) to be solid or empty based on setSolid parameter
