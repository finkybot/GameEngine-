...
private:
	// PackChunkID - Packs the chunk coordinates (cx, cy) into a single 64-bit integer for use as a key in the chunk map. This allows for efficient storage and retrieval of chunks based on their coordinates.
	...
	// Map of chunk IDs to chunk entities for quick lookup and management of loaded chunks. The key is a 64-bit integer representing the packed chunk coordinates, and the value is a pointer to the corresponding chunk entity.
	std::unordered_map<uint64_t, Entity*> m_chunkMap;

	// Configuration for chunk dimensions and tile size. These values determine how many tiles are in each chunk and the size of each tile in world units.
