...
struct BoundingBox
{
	...
	BoundingBox(const Vec2& topLeft, const Vec2& bottomRight)	// Constructor to initialize the bounding box with specific top-left and bottom-right points
	{
	...
	Vec2 GetBottomRightPoint() const noexcept { return bottomRight; }					// Getter for the bottom-right point of the bounding box
	float GetHeight() const noexcept { return bottomRight.GetY() - topLeft.GetY(); }	// Getter for the height of the bounding box, calculated as the difference in y-coordinates between the bottom-right and top-left points
	float GetWidth() const noexcept { return bottomRight.GetX() - topLeft.GetX(); }		// Getter for the width of the bounding box, calculated as the difference in x-coordinates between the bottom-right and top-left points
};
...
private:
	...
	unsigned int					m_capacity;																// The maximum number of objects this node can hold before it needs to subdivide into child nodes
	...
	std::array<std::unique_ptr<QuadTree<T>>, 4> m_childTreePtrs{ { nullptr, nullptr, nullptr, nullptr } };	// Unique pointers to the four child QuadTree nodes (top-left, top-right, bottom-left, bottom-right), initialized to nullptr
	unsigned int					m_depth = 0;															// The depth of this node in the QuadTree hierarchy, with the root node at depth 0. This is used to limit the maximum depth of subdivision and prevent infinite recursion.
	unsigned int					m_maxDepth = std::numeric_limits<unsigned int>::max();					// The maximum depth allowed for subdivision of the QuadTree. This prevents infinite subdivision and can be set to a specific value based on the expected density of objects and performance requirements.
	QuadTree<T>* m_parent = nullptr;																		// Pointer to the parent QuadTree node, used for potential upward traversal if needed (not currently utilized in this implementation but can be useful for certain operations like merging child nodes back into a parent)

	// Query instrumentation
	inline static size_t			s_totalQueriesThisFrame = 0;	// Static variable to track the total number of queries made in the current frame, used for performance monitoring and debugging purposes. It is incremented each time a query is performed on the QuadTree, allowing developers to analyze how many spatial queries are being made and optimize accordingly.
	inline static size_t			s_totalObjectsQueried = 0;		// Static variable to track the total number of objects queried in the current frame, used for performance monitoring and debugging purposes.
	inline static size_t			s_totalNodesVisited = 0;		// Static variable to track the total number of QuadTree nodes visited during queries in the current frame, used for performance monitoring and debugging purposes. It is incremented each time a node is visited during a query, allowing developers to analyze the efficiency of the QuadTree structure and optimize it if necessary.
	inline static size_t			s_queryCount = 0;				// Static variable to track the total number of queries performed on the QuadTree, used for performance monitoring and debugging purposes. It is incremented each time a query is performed, allowing developers to analyze how many spatial queries are being made and optimize accordingly.

	// Helper function to insert raw pointer
	bool InsertRawPointer(T* object) // This method attempts to insert an object into the QuadTree. It first checks if the object's center point is within the boundary of this node. If it is not, the method returns false, indicating that the object cannot be inserted into this node. If the object is within the boundary and there is capacity in this node (i.e., the number of objects currently in this node is less than the defined capacity), the object is added to this node's list of objects and the method returns true. If there is no capacity and this node has not yet been subdivided, it will subdivide itself into four child nodes (if it has not reached the maximum depth) and then attempt to insert the object into one of the child nodes. If the object cannot be inserted into any of the child nodes (e.g., if it lies on a boundary or if all child nodes are at capacity), it will be added to this node's list of objects as a fallback.
	{
		...
		else // If there is no capacity and this node has not yet been subdivided, it will subdivide itself into four child nodes (if it has not reached the maximum depth) and then attempt to insert the object into one of the child nodes. If the object cannot be inserted into any of the child nodes (e.g., if it lies on a boundary or if all child nodes are at capacity), it will be added to this node's list of objects as a fallback.
		{
			...
			if (m_isSubDivided)
			{
				...
				m_objects.push_back(object); // If the object cannot be inserted into any of the child nodes (e.g., if it lies on a boundary or if all child nodes are at capacity), add the object to this node's list of objects as a fallback and return true.
				return true;
			}
		}
