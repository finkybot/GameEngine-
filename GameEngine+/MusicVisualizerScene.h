...
private:
	...
	Entity* m_tileMapEntity =
		nullptr; // Entity representing the tile map in the scene, used for rendering and potential interactions
	bool m_enableImGui =
		true; // Flag to enable or disable ImGui rendering for this scene, allowing for a cleaner visual if desired
	std::filesystem::path
		m_currentDir; // Current directory for file browsing, initialized to the executable's directory for convenience when loading music files
	float m_fps =
		0.0f; // Variable to store the current frames per second (FPS) for display in the UI, updated each frame in the Update() method
