...
public:
	...
	void RenderAliveEntities(
		const std::vector<std::unique_ptr<Entity>>& entities,
		sf::RenderWindow&
			window); // Renders all alive entities to the provided SFML render window (why was you missing????)
	...
	void RenderAll(const std::vector<std::unique_ptr<Entity>>& entities, sf::RenderWindow& window,
				   RenderMode mode = RenderMode::ShapesThenText);

	// Render a single entity (shape and text if present).
	void RenderEntity(Entity* entity, sf::RenderWindow& window)
		const; // Helper function that renders a single entity (shape and text if present), called by the public Render method.
