...
/////////////////////////////////
//	|	Forward declarations to avoid header cycles. We forward declare the SFML RenderWindow class and the Entity class, as well as the FontManager class which is used for text rendering. This allows us to use pointers or references to these types without including their full definitions in this header, which can help reduce compilation 
...
class RenderSystem {
public:
	// Constructor and destructor for the RenderSystem class. The default constructor and destructor are sufficient since we don't have any member variables that require special initialization or cleanup. If we add member variables in the future that require custom handling, we can implement these methods accordingly.
	RenderSystem() = default; 
	~RenderSystem() = default;



	// RenderAliveEntities - Renders all alive entities to the provided SFML render window. This method iterates through the list of entities, checks if they are alive, and draws their shapes and text components if present. It serves as the main entry point for rendering entities in the game loop.
