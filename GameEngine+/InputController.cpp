...
void InputController::Update(uint32_t deltaT)
{
	while (const std::optional<sf::Event> event = m_window->pollEvent())	
	{
		...
		if (const auto* mouseButton = event->getIf<sf::Event::MouseButtonPressed>())
		{
			if (m_CurrentController)
			{
				MouseInputAction action = m_CurrentController->GetActionForMouseButton(static_cast<MouseButton>(mouseButton->button));
		...
		if (const auto* keyPressed = event->getIf<sf::Event::KeyPressed>())
		{
			if (m_CurrentController)
			{
				std::cout << "Key Pressed Event Detected: " << static_cast<int>(keyPressed->code) << std::endl; // Debugging output
