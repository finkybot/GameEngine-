...
static bool parse_json_layers(const std::string& s, std::vector<TileMap::Layer>& out) {
    ...
    while (cur < s.size()) {
        ...
        if (nextComma == std::string::npos || nextComma > nextClose) break;
    }
    return !out.empty();
}
...
bool TileMap::SaveToJSON(const std::string& path, std::string* outErr) const
{
    ...
    if (!tilesetKey.empty()) os << "  \"tilesetKey\": \"" << tilesetKey << "\",\n";
    if (!tilesetImage.empty()) os << "  \"tilesetImage\": \"" << tilesetImage << "\",\n";
    if (tilesetTileW > 0) os << "  \"tilesetTileW\": " << tilesetTileW << ",\n";
    if (tilesetTileH > 0) os << "  \"tilesetTileH\": " << tilesetTileH << ",\n";
    ...
    if (!layers.empty()) {
        ...
        for (size_t li = 0; li < layers.size(); ++li) {
            ...
            os << "]\n";
            os << "    }";
            if (li + 1 < layers.size()) os << ",\n"; else os << "\n";
        }
        os << "  ],\n";
    } else {
        os << "  \"tiles\": [";
        const int total = width * height;
        for (int i = 0; i < total; ++i) { if (i) os << ", "; os << tiles[i]; }
        os << "]\n";
    }
    os << "}\n";
    return true;
