...
/////////////////////////////////
//	|	BoundingBox struct - Represents an axis-aligned bounding box defined by its top-left and bottom-right corners. It provides methods for calculating the center point, checking if a point is contained within the box, and checking for intersection with another bounding box. The class uses half-open intervals [topLeft, bottomRight) 
...
struct BoundingBox {
	...
	// GetCentrePoint - Calculates and returns the center point of the bounding box by averaging the x and y coordinates of the top-left and bottom-right corners. This method provides a convenient way to get the central position of the bounding box, which can be useful for various spatial calculations and queries.
	Vec2 GetCentrePoint() const noexcept {
		return Vec2((topLeft.GetX() + bottomRight.GetX()) * 0.5f, (topLeft.GetY() + bottomRight.GetY()) * 0.5f);
	}


	// ContainsPoint - Checks if a given point is contained within the bounding box. The method uses half-open intervals [topLeft, bottomRight) for containment checks, meaning that points on the left and top edges are considered inside the box, while points on the right and bottom edges are considered outside. 
	// This approach avoids ambiguity when checking points that lie exactly on the boundaries of the box.
...
/////////////////////////////////
//	|	QuadTree class template - A spatial partitioning data structure that recursively subdivides a 2D space into four quadrants (child nodes) to efficiently manage and query objects based on their spatial location. The QuadTree is designed to hold pointers to objects of type T, which are expected to have a method 
//	|	GetCentrePoint() that returns their position in the 2D space. The QuadTree supports insertion of objects, querying for objects within a specified area, updating object positions, and removing objects from the tree. It also includes instrumentation for tracking query performance metrics such as the total number 
//	|	of queries, total objects queried, and total nodes visited during queries.
//	|_______________________________________________________________________
template <typename T>
class QuadTree {
private:
	// The boundary of this QuadTree node, represented as a BoundingBox. This defines the spatial area that this node covers in the 2D space. Objects that fall within this boundary can be stored in this node or its child nodes, while objects outside this boundary cannot be stored in this node.
	BoundingBox m_boundary;

	// The capacity of this QuadTree node, which determines how many objects can be stored in this node before it needs to subdivide into child nodes. This is a critical parameter that affects the performance of the QuadTree, as a smaller capacity will lead to more subdivisions and potentially deeper trees, while a 
	// larger capacity may lead to fewer subdivisions but more objects stored in each node, which can affect query performance.
	...
	// Flag to indicate whether this QuadTree node has been subdivided into child nodes. If this flag is false, it means that this node is a leaf node and can store objects directly. If this flag is true, it means that this node has been subdivided into four child nodes (top-left, top-right, bottom-left, bottom-right), 
	// and objects should be stored in the appropriate child node based on their position.
	...
	// The maximum depth allowed for subdivision of the QuadTree. This prevents infinite subdivision and can be set to a specific value based on the expected density of objects and performance requirements. If the maximum depth is reached, the node will not subdivide further and will store objects directly, even if it exceeds 
	...
	// InsertRawPointer - A helper method for inserting an object into the QuadTree using a raw pointer. This method is responsible for checking if the object's center point is within the boundary of this node, determining whether to store the object in this node or to subdivide and insert it into child nodes, and handling cases where 
	// the object cannot be inserted into child nodes. The method returns true if the object was successfully inserted into this node or one of its child nodes, or if it was added to this node's list of objects as a fallback, and false if the object cannot be inserted into this node (e.g., if it is outside the boundary).
...
public:
	...
	// ClearTree - A method to clear the QuadTree by recursively clearing all child nodes and resetting the state of this node. This method is called in the destructor to ensure that all resources are properly released when the QuadTree is destroyed. It also clears the list of objects for this node.
	void ClearTree() {
		if (m_isSubDivided) {
			for (auto& child : m_childTreePtrs) {
				if (child)
	...
	// GetBoundary - A method to retrieve the boundaries of this QuadTree node and all of its child nodes. This method takes a reference to a vector of BoundingBox objects and appends the boundary of this node to the vector, then recursively calls GetBoundary on each child node (if subdivided) to collect their boundaries as well.
	...
	// Insert - A public method to insert an object into the QuadTree using a pointer. This method is a wrapper around the InsertRawPointer helper method, which performs the actual insertion logic. The Insert method returns true if the object was successfully inserted into this node or one of its child nodes,
	bool Insert(T* object) { return InsertRawPointer(object); }


	// SubDivide - A method to subdivide this QuadTree node into four child nodes (top-left, top-right, bottom-left, bottom-right) based on the current boundary of this node. This method is called when the capacity of this node is exceeded and it needs to create child nodes to store additional objects.
	void SubDivide() {
	...
	// Query - An overloaded version of the Query method that takes an additional parameter for the current object being queried. This allows the method to exclude the current object from the results, which can be useful in scenarios where you want to find nearby objects without including the object itself in the results.
	...
	// Size - A method to calculate and return the total number of objects stored in this QuadTree node and all of its child nodes. This method recursively counts the number of objects in this node and adds it to the counts from each child node (if subdivided) to get the total size of the QuadTree.
	size_t Size() {
	...
	// ResetQueryStats - A static method to reset the query performance metrics tracked by the QuadTree. This method sets the total number of queries, total objects queried, total nodes visited, and query count back to zero. It can be called at the beginning of each frame or query session to track metrics for specific time periods.
	...
	// RemoveEntityFromTree - A method to remove an object from the QuadTree using a pointer. This method calls the RemoveObjectFast helper method to remove the object from this node's list of objects, and then recursively calls RemoveEntityFromTree on each child node (if subdivided) to ensure that the object is removed 
