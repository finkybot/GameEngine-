...
public:
	...
	void SetColor(
		...
		int alpha); // Set the fill color of the explosion shape using RGBA values (alpha is an integer in the range [0, 255])
	void SetRadius(float radius) override {
		m_circle.setRadius(radius);
		m_circle.setOrigin(sf::Vector2f(radius, radius));
	} // Set the radius of the explosion shape and update origin
};