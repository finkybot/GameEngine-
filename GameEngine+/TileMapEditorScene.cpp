...
void TileMapEditorScene::Update(float deltaTime) {
	...
	if (GImGui && GImGui->WithinFrameScope) {
		...
		ImGui::Begin("TileMap Editor", nullptr,
		...
		if (ImGui::Button("Save")) {
			...
			if (m_saveFilenameBuffer[0] == '\0') {
				std::string pre = m_currentDir.string();
				pre += std::filesystem::path::preferred_separator;
				pre += (m_currentFilename.empty() ? "map.json"
												  : std::filesystem::path(m_currentFilename).filename().string());
		...
		if (ImGui::BeginPopupModal("Save Map", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
			...
			if (ImGui::Button("OK")) {
				...
				if (!path.empty()) {
					std::string err;
					if (m_tileMap.SaveToJSON(path, &err)) {
		...
		auto refresh_entries = [&]() {
			...
			try {
				...
				std::sort(s_entries.begin(), s_entries.end(), [](auto const& a, auto const& b) {
					...
					if (a_dir != b_dir)
						return a_dir > b_dir; // directories first
		...
		if (ImGui::Button("Load adventure.png into atlas")) {
			std::vector<std::filesystem::path> candidates = {
				m_currentDir / "adventure.png", m_currentDir / "assets" / "adventure.png",
				std::filesystem::current_path() / "assets" / "adventure.png",
				std::filesystem::current_path() / "adventure.png"};
			std::string found;
			std::error_code ec;
			for (auto& c : candidates) {
				if (!c.empty() && std::filesystem::exists(c, ec) && !ec) {
					found = c.string();
					break;
				}
			}
			if (found.empty()) {
				...
				if (ok)
					std::cout << "Texture atlas loaded: " << found << std::endl;
				else
					std::cerr << s_atlas_status << std::endl;
				if (ok) {
		...
		if (!s_atlas_status.empty())
			ImGui::TextUnformatted(s_atlas_status.c_str());
		...
		if (ImGui::CollapsingHeader("Tile Entities (Textures)")) {
			...
			for (Entity* te : tileEntities) {
				...
				if (!tileRectOpt.has_value() || !texPtr)
					continue;
				auto tr = *tileRectOpt;
			...
			int texturedCount = 0;
			for (Entity* te : tileEntities)
				if (te && te->GetComponent<CTexture>())
					++texturedCount;
		...
		if (ImGui::Button("Load assets/ambition.mp3")) {
			std::vector<std::filesystem::path> candidates = {
				m_currentDir / "ambition.mp3", m_currentDir / "assets" / "ambition.mp3",
		...
		if (!s_music_status.empty())
			ImGui::TextUnformatted(s_music_status.c_str());
		...
		if (m_audioReactive && level > m_spawnThreshold) {
			if (m_spawnTimer >= m_spawnCooldown) {
				...
				if (e) {
					...
					auto* t = e->AddComponent<CTransform>(Vec2(x, y),
														  Vec2(0.0f, -40.0f - static_cast<float>(std::rand() % 120)));
					...
					std::cout << "AudioReactive: Spawned entity id=" << e->GetId() << " size=" << size << " pos=(" << x
							  << "," << y << ")" << std::endl;
					m_spawnTimer = 0.0f;
				}
			}
...
void TileMapEditorScene::ProcessInput() {
	...
	bool ctrlS =
		sf::Keyboard::isKeyPressed(sf::Keyboard::Key::LControl) && sf::Keyboard::isKeyPressed(sf::Keyboard::Key::S);
	...
	bool ctrlShiftS = sf::Keyboard::isKeyPressed(sf::Keyboard::Key::LControl) &&
