...
/////////////////////////////////
//	|	LevelEditorScene class - implements a chunked level editor scene with camera controls, tile editing, and asynchronous chunk loading. The scene manages a grid of chunks that can be edited in real-time, with support for panning the camera and loading/unloading chunks as needed to optimize performance.
...
public:
	...
	// Overridden virtual methods from the Scene base class. These methods handle updating the scene state, rendering the scene, processing input events, and managing the scene lifecycle (entering and exiting). The Update method will handle logic for ensuring visible chunks are loaded and applying camera view, while the Render 
	// method will handle drawing the chunks and any UI elements.
...
private:
	...
	// Temporary storage for render shapes during Enqueue phase (kept alive until Flush completes) Use a map with stable memory addresses instead of a vector to avoid pointer invalidation
