...
public:
	...
	// Update - Method that should be called every frame to manage music playback based on the state of CMusic components in the entities. This method will handle starting new music tracks, stopping tracks that are no longer active, and updating any necessary state for currently playing tracks (e.g., measuring levels, performing analysis).
	...
	// StopAllMusic - Method to stop all currently playing music tracks. This can be called when transitioning between scenes, when the player dies, or in any situation where you want to immediately stop all music playback. It will iterate through all active music instances and call their stop method, then clear the active music mapping.
	void StopAllMusic();


	// GetLevel - Method to query the latest measured RMS level for a given entity ID. This can be used for visualizations (e.g., volume meters) or gameplay mechanics that depend on the current audio level of a music track. It returns a float value between 0.0 and 1.0 representing the normalized RMS level, where 0.0 means silence 
	// and 1.0 means maximum volume.
	float GetLevel(size_t entityId) const;


	// HasAnalysisBuffer - Method to check if an analysis buffer (sf::SoundBuffer) is available for a given entity ID. This can be used to determine if spectral analysis or other audio processing features are available for a music track, based on whether the corresponding sound buffer was successfully loaded.
	bool HasAnalysisBuffer(size_t entityId) const;


	// GetPlayingOffset - Method to query the current playback position (in seconds) for a given entity ID. This can be used for visualizations (e.g., progress bars) or gameplay mechanics that depend on the current position of a music track. It returns a float value representing the playback offset in seconds, or 0.0 if the music 
	// is not available or not currently playing.
	...
	// Seek - Method to seek to a given offset (in seconds) for a given entity ID. This can be used to implement features like scrubbing through a music track or starting playback from a specific position. No-op if music is not available.
	void Seek(size_t entityId, float seconds);


	// Spectrum (Goertzel) accessors -
	// GetSpectrum - Method to query the latest calculated spectrum values for a given entity ID. This can be used for visualizations (e.g., spectrum analyzers) or gameplay mechanics that depend on the current spectral content of a music track. It fills the provided vector with the latest spectrum values, where each value represents 
	// the magnitude of a specific frequency band.
...
private:
	// Map of entity ID to latest measured RMS level for that entity's music track. This allows us to provide level information for visualizations or gameplay mechanics that depend on the current audio level of a music track. The levels are normalized to a range of 0.0 to 1.0, where 0.0 means silence and 1.0 means maximum volume.
	...
	// Spectral analysis storage (Goertzel bands) Map of entity ID to vector of latest calculated spectrum values for that entity's music track. This allows us to provide spectrum information for visualizations or gameplay mechanics that depend on the current spectral content of a music track, protected by m_levelsMutex.
	...
	// Playhead snapshot written by Process() (main thread) and read by the analysis thread. Protected by m_playheadMutex to avoid tearing.
