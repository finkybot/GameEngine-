		...
		if (m_lmbdragging) {
			...
			for (float rayDistance = 0.0f; rayDistance <= dragLen; rayDistance += step) {
				...
				if (std::find(m_visitedCells.begin(), m_visitedCells.end(), std::pair<int, int>(cellX, cellY)) ==
					m_visitedCells.end()) {
					m_visitedCells.push_back({cellX, cellY});
				}
			}
...
void TileMapScene::ProcessMouseRightDrag(bool& rightMouseDown, const Vec2& mouseWorld) {
	...
	// and calculate the drag direction vector, although for tile toggling we may not need the direction.
		...
		if (m_rmbdragging) {
			...
			m_currentTile = Vec2(static_cast<int>(std::floor(m_rmbDragStart.x / m_tileMap.tileSize)),
								 static_cast<int>(std::floor(m_rmbDragStart.y / m_tileMap.tileSize)));
			//ToggleTileAt(currentTile.x, currentTile.y);
		}
	}
...
void TileMapScene::DrawDebugLines() {
	...
	for (const auto& pr : m_debugLines) {
		sf::Vertex line[] = {sf::Vertex(sf::Vector2f(pr.first.x, pr.first.y), sf::Color::Red),
...
void TileMapScene::SpawnTestTileMap() {
	...
	for (int x = 3; x + 3 < cols - 3; x += 14) {
		if (groundY - 1 >= 0)
			map.SetTile(x, groundY - 1, 1);
		if (groundY - 2 >= 0)
			map.SetTile(x, groundY - 2, 1);
	}
...
RaycastHit TileMapScene::MakeStartCellHit(int tileX, int tileY, const Vec2& origin) {
	RaycastHit h;
	if (tileX < 0 || tileY < 0)
		return h; // Guard: invalid cell, return no hit
