...
void TestScene::InitializeGame(sf::Vector2u windowSize) {
	int maxEntities =
		5000; // Target entity population to maintain in the scene (it won't reach this number as to many will be murdered in collisions, but still should be a lively scene)
	...
	m_xDistro = std::uniform_int_distribution<int>(
		20,
		m_gameEngine.m_windowSize.x -
			20); // Spawn within screen bounds, leaving a 20-pixel margin on the left and a 5-pixel margin on the right to prevent immediate off-screen spawning
	m_yDistro = std::uniform_int_distribution<int>(
		20,
		m_gameEngine.m_windowSize.y -
			20); // Spawn within screen bounds, leaving a 20-pixel margin on the top and a 5-pixel margin on the bottom to prevent immediate off-screen spawning
...
void TestScene::UpdateExplosions() {
	...
	for (
		auto& entity :
		m_entityManager
			.getEntities()) { // Iterate over all entities to find explosions and update their state based on elapsed time since creation
