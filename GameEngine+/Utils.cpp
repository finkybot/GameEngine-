...
static bool parse_json_int_array_from_pos(const std::string& s, size_t startBracketPos, std::vector<int>& out, size_t& outPos) {
	if (startBracketPos >= s.size() || s[startBracketPos] != '[') return false;
	size_t pos = startBracketPos + 1;
	out.clear();
	while (pos < s.size()) {
		while (pos < s.size() && isspace((unsigned char)s[pos])) ++pos;
		if (pos >= s.size()) break;
		if (s[pos] == ']') { outPos = pos + 1; return true; }
		bool neg = false;
...
static bool parse_json_layers(const std::string& s, std::vector<TileMap::Layer>& out) {
	...
	while (cur < s.size()) {
		...
		std::string name;
		if (parse_json_string(s, "name", name, objStart)) layer.name = name;
		...
		if (tilesPos != std::string::npos && tilesPos < objEnd) {
			...
			if (bracket != std::string::npos && bracket < objEnd) {
				size_t after;
				if (!parse_json_int_array_from_pos(s, bracket, layer.tiles, after)) return false;
			}
		}
		...
		if (nextComma == std::string::npos || nextComma > nextClose) break; // no more objects
	}
	return !out.empty();
}
...
bool SaveTileMapJSON(const TileMap& map, const std::string& path, std::string* outErr)
{
    ...
    // tileset metadata
	if (!map.tilesetKey.empty()) os << "  \"tilesetKey\": \"" << map.tilesetKey << "\",\n";
	if (!map.tilesetImage.empty()) os << "  \"tilesetImage\": \"" << map.tilesetImage << "\",\n";
	if (map.tilesetTileW > 0) os << "  \"tilesetTileW\": " << map.tilesetTileW << ",\n";
	if (map.tilesetTileH > 0) os << "  \"tilesetTileH\": " << map.tilesetTileH << ",\n";
	...
	if (!map.layers.empty()) {
		...
		for (size_t li = 0; li < map.layers.size(); ++li) {
			...
			for (int i = 0; i < total; ++i) { if (i) os << ", "; os << layer.tiles[i]; }
			os << "]\n";
			os << "    }";
			if (li + 1 < map.layers.size()) os << ",\n"; else os << "\n";
		}
		os << "  ],\n";
	} else {
		...
		os << "  \"tiles\": [";
		const int total = map.width * map.height;
		for (int i = 0; i < total; ++i) { if (i) os << ", "; os << map.tiles[i]; }
		os << "]\n";
	}
	os << "}\n";
	return true;
	...
	if (parse_json_layers(s, layers)) {
		...
		if (!layers.empty()) {
			if ((int)layers[0].tiles.size() != width * height) {
				if (outErr) *outErr = "Layer 0 tiles count does not match width*height";
				return std::nullopt;
			}
			...
			map.tilesetTileW = tw; map.tilesetTileH = th;
			return map;
		}
	}
