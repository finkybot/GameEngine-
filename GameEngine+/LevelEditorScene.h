...
public:
	/////////////////////////////////
	// Constructor and destructor for the LevelEditorScene class. The constructor initializes the scene with references to the game engine, render window, and entity manager, 
	// while the destructor ensures that all chunks are saved to disk when the scene is destroyed.
	...
	/////////////////////////////////
	// Private helper methods for the LevelEditorScene class. These methods include logic for ensuring that visible chunks are loaded based on the camera view, 
	// applying the main camera's view to the render window, and processing user input for camera controls and tile editing.
private:
	/////////////////////////////////
	...
	/////////////////////////////////
	// AtlasLoadWorker - Worker function used to load a texture atlas on a background thread. This function takes the key, file path, and tile 
	// dimensions for the atlas, and will load the texture and create the necessary sub-rectangles for each tile in the atlas.
	...
	/////////////////////////////////
	// Guard that blocks any painting until all mouse buttons have been fully released at least once after entering the scene. Prevents the button used to 
	// open the scene (e.g. a main-menu click) from immediately starting a selection drag.
