...
struct TileMap {
	...
	// GetTile - retrieves the value of a tile at the specified coordinates if they are within bounds, returning 0 (empty) if the coordinates are out of bounds. This method uses the InBounds method to check if the coordinates are valid before accessing the tile data, providing a safe way to query tile values without risking out-of-bounds access.
	inline int GetTile(int x, int y) const { return InBounds(x, y) ? tiles[y * width + x] : 0; }


	// SetTile - updates the value of a tile at the specified coordinates if they are within bounds. This method checks if the given coordinates are valid using the InBounds method before modifying the tile value, ensuring that tile updates do not cause out-of-bounds access or modify invalid memory.
	...
	// IsSolid - checks if a tile is solid (non-zero) at the specified coordinates, returning false if the coordinates are out of bounds. This method uses the GetTile method to retrieve the tile value and determine if it is solid, providing a convenient way to check for solid tiles in the tile map.
	inline bool IsSolid(int x, int y) const { return GetTile(x, y) != 0; }


	// SaveToJSON - saves the tile map data to a JSON file at the specified path. This method serializes the tile map properties and tile data into a JSON format, allowing for easy storage and retrieval of tile maps. The method returns true if the save operation was successful, or false if an error occurred, 
	// with an optional output parameter for error messages.
	bool SaveToJSON(const std::string& path, std::string* outErr = nullptr) const;


	// LoadFromJSON - loads tile map data from a JSON file at the specified path, returning an optional TileMap object. This method deserializes the JSON data to populate the tile map properties and tile data, allowing for easy loading of tile maps from files. If the load operation is successful, it returns a 
	// TileMap object; if an error occurs, it returns std::nullopt, with an optional output parameter for error messages.
