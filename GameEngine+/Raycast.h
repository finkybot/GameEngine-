...
static inline bool NormalizeDir(const Vec2& d, double& outDx, double& outDy, double& outLen) {
	...
	outDx = d.x;
	outDy = d.y;
	outLen = std::sqrt(outDx * outDx + outDy * outDy);
	if (outLen < EPS_SMALL)
		return false;
	outDx /= outLen;
	outDy /= outLen;
...
inline RaycastHit RaycastTilemapDDA(const Vec2& origin, const Vec2& dir, const TileMap& map, float maxDistance,
		...
		if (std::fabs(tMaxX - tMaxY) < CORNER_EPS) {
			...
			if (map.IsSolid(mapX, mapY)) {
				...
				// If the intersection test fails due to precision issues, we can still report a hit at the corner t value, which is the best estimate we have for the hit distance in this case.
					...
					result.position = Vec2(origin.x + static_cast<float>(dxN * cornerT),
										   origin.y + static_cast<float>(dyN * cornerT));
				}
				result.hit = true;
				result.tileX = mapX;
				result.tileY = mapY;
		...
		if (map.IsSolid(mapX, mapY)) {
			...
			if (IntersectAABB_Ray(origin, dxN, dyN, map, mapX, mapY, maxDistance, outHitDistance)) {
				...
				if (std::fabs(hitP.x - mapX * map.tileSize) < EPS_FACE)
					...
					result.normal = Vec2(1.0f, 0.0f);
				else if (std::fabs(hitP.y - mapY * map.tileSize) < EPS_FACE)
					result.normal = Vec2(0.0f, -1.0f);
				else if (std::fabs(hitP.y - (mapY + 1) * map.tileSize) < EPS_FACE)
					result.normal = Vec2(0.0f, 1.0f);
				else
					result.normal =
						(stepXNext ? Vec2((dxN > 0) ? -1.0f : 1.0f, 0.0f) : Vec2(0.0f, (dyN > 0) ? -1.0f : 1.0f));
	...
	if (map.InBounds(endMapX, endMapY) && map.IsSolid(endMapX, endMapY)) {
		...
		if (IntersectAABB_Ray(origin, dxN, dyN, map, endMapX, endMapY, maxDistance, outHitDistance)) {
			result.hit = true;
			result.tileX = endMapX;
			result.tileY = endMapY;
			result.tileValue = map.GetTile(endMapX, endMapY);
			result.distance = outHitDistance;
			result.position = Vec2(origin.x + static_cast<float>(dxN * outHitDistance),
								   origin.y + static_cast<float>(dyN * outHitDistance));
			return result;
		}
